...
public class FinalDb {
	...
	private synchronized static FinalDb getInstance(DaoConfig daoConfig) {
		Loger.e("version", daoConfig.getDbVersion() + "");
		FinalDb dao = daoMap.get(daoConfig.getDbName()
				+ daoConfig.getDbVersion());
		if (dao == null) {
			dao = new FinalDb(daoConfig);
			daoMap.put(daoConfig.getDbName() + daoConfig.getDbVersion(), dao);
		}
		return dao;
	}
	...
	public static FinalDb create(Context context, String dbName) {
		...
		config.setDbVersion(MyApplication.dbVersion);
	 ...
	 * @param strSQL
	 */
	public <T> List<T> rawQueryBySql(Class<T> clazz, String strSQL,
			String[] args) {
		checkTableExist(clazz);
		debugSql(strSQL);
	...
	public static class DaoConfig {
		...
		private int dbVersion = MyApplication.dbVersion; // 数据库版本
		private boolean debug = true; // 是否是调试模式（调试模式 增删改查的时候显示SQL语句）
		private DbUpdateListener dbUpdateListener;
		private boolean saveOnSDCard = true;// 是否保存到SD卡
		...
		public int getDbVersion() {
			return MyApplication.dbVersion;
		}
	...
	class SqliteDbHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			Loger.e("SqliteDbHelper", "onCreate");
		}
